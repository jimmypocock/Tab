...
import { ArrowRight, Code, CreditCard, FileText, Zap, User } from 'lucide-react'
...
export function HomePageContent({ isAuthenticated, userEmail }: HomePageContentProps) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [isProcessing, setIsProcessing] = useState(false)

  useEffect(() => {
    // Log all parameters to see what Supabase sends
    if (searchParams.toString()) {
      console.log('URL params after email confirmation:', Object.fromEntries(searchParams))
    }
    
    const code = searchParams.get('code')
    
    // For now, just check for code parameter
    // In production, Supabase will respect our emailRedirectTo setting
